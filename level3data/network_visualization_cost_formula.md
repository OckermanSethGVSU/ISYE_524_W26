...
Only `LRU_*` and `LRU_*_*` components are repairable at `--complexity 3`. Shared `PART_*` items are used to build repairs but are not themselves repair options.
...
| --- | --- | --- |
| `specialized_shop` | 2.0 | 0.8 |
| `local_shop` | 1.2 | 0.95 |
| `regional_warehouse` | 1.0 | 1.1 |
| `central_depot` | 0.85 | 3.0 |
